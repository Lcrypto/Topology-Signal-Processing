...
function info = hSSBurstInfo(burst)
    ...
    % to maximise the flexibility of SS burst placement
    nfft = burst.SampleRate / (gnb.SubcarrierSpacing * 1e3);
    gnb.NRB = floor(nfft * 0.85 / 12);
    if (gnb.NRB < 20)
        error('Maximum NRB supported by sample rate (%0.2f MHz) and SSB subcarrier spacing (%d kHz) is %d, but SSB requires at least NRB=20.',burst.SampleRate/1e6,gnb.SubcarrierSpacing,gnb.NRB);
    end
    ...
    gridFreqSpan = gnb.NRB * 12 / 2 * gnb.SubcarrierSpacing * 1e3;
    if ((burst.FrequencyPointA < -gridFreqSpan) || (burst.FrequencyPointA > (gridFreqSpan - (burst.SubcarrierSpacingCommon * 1e3))))
        error('Resource grid (NRB = %d, spanning %0.3f .... %0.3f MHz) configured by sample rate (%0.2f MHz) and SSB subcarrier spacing (%d kHz) does not span FrequencyPointA (%0.3f MHz).',gnb.NRB,-gridFreqSpan/1e6,(gridFreqSpan - (gnb.SubcarrierSpacing*1e3))/1e6,burst.SampleRate/1e6,gnb.SubcarrierSpacing,burst.FrequencyPointA/1e6);
    end
    ...
    ssbf0 = burst.FrequencySSB - (120 * gnb.SubcarrierSpacing * 1e3);
    ...
    kStep = gnb.SubcarrierSpacing / k_SSB_units;
    ...
    delta_f = mod(ssbf0,gnb.SubcarrierSpacing * 1e3);
    ssbf0 = ssbf0 - delta_f;
    
    % Calculate offset to the subcarrier in the resource grid which 
    % represents SSB subcarrier k=0
    k0_offset = ofdmInfo.NSubcarriers/2 + (ssbf0 / (gnb.SubcarrierSpacing * 1e3));
    if ((k0_offset < 0) || (k0_offset > (ofdmInfo.NSubcarriers - 240)))
        error('Resource grid (NRB = %d, spanning %0.3f ... %0.3f MHz) configured by sample rate (%0.2f MHz) and SSB subcarrier spacing (%d kHz) does not span SSB (%0.3f ... %0.3f MHz) configured by FrequencySSB (%0.3f MHz).',gnb.NRB,-gridFreqSpan/1e6,(gridFreqSpan - (gnb.SubcarrierSpacing*1e3))/1e6,burst.SampleRate/1e6,gnb.SubcarrierSpacing,(burst.FrequencySSB - 120 * gnb.SubcarrierSpacing *1e3)/1e6,(burst.FrequencySSB + 119 * gnb.SubcarrierSpacing *1e3)/1e6,burst.FrequencySSB/1e6);
    end
    ...
    info.NRB = gnb.NRB;
    info.CyclicPrefix = gnb.CyclicPrefix;
