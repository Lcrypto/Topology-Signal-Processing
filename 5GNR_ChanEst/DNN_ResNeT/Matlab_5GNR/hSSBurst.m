...
%   The following additional fields relate to the constructon of the OFDM
...
function [waveform,grid,info] = hSSBurst(burst)
    ...
    gnb.NRB = info.NRB;
    gnb.SubcarrierSpacing = info.SubcarrierSpacing;
    gnb.CyclicPrefix = info.CyclicPrefix;
    ...
    % Modulate grid
    waveform = hOFDMModulate(gnb,grid);
    ...
    if (burst.DisplayBurst)
        displayBurst(gnb,burst,displaygrid);
    end
    
end

function displayBurst(gnb,burst,displaygrid)
    
    figure;
    
    [K,L] = size(displaygrid);
    imagesc(1:L,((1:K)-(K/2))*gnb.SubcarrierSpacing/1e3,displaygrid);
    
    axis xy;
    ylabel('Frequency (MHz)');
    xlabel('OFDM symbols');
    title(sprintf('SS burst, block pattern = %s, SCS = %d kHz',burst.BlockPattern,gnb.SubcarrierSpacing));
